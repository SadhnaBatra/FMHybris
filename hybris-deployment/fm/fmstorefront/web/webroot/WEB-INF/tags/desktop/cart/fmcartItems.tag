...
<table id="myTable" class="table tablesorter">
	<thead>
		<tr>
			...
			<th class="col-md-1 text-center"><spring:theme code="basket.page.priceType"/></th>
	...
	<tbody>
			...
			<tr>
				...
				<td class="col-md-8  col-xs-8" colspan="2">
					<div class="shoppingCartsubHead">
						...
						<div class="">
							<span><span class=""><spring:theme code="cart.page.partNumber"/></span>&nbsp;${entry.product.partNumber} </span>
					...
					<div class="clearfix">
						<div class="pull-left">
							...
							<form:form id="updateCartForm${entry.entryNumber}" action="${cartUpdateFormAction}" method="post" commandName="updateQuantityForm${entry.entryNumber}">
								...
								<input type="hidden" name="productPrice" value="${entry.basePrice.value}"/>
									...
									<c:when test="${not entry.product.multidimensional}">
										<ycommerce:testId code="cart_product_quantity">
											...
											<c:if test="${entry.updateable}" >
												<form:input disabled="${not entry.updateable}" type="text" maxlength="5" size="1" id="quantity${entry.entryNumber}" class="form-control width58 btmMrgn_14" path="quantity" onkeypress="return isNumber(event);"/>
											</c:if>
										</ycommerce:testId>
							...
							<c:if test="${entry.updateable}" >
								<ycommerce:testId code="cart_product_removeProduct">
									<div class="addNewAddLink shoppingCartQuantityLink shoppingCartQuantityLinkMargin pull-left lftMrgn_5">
										<a href="#" id="RemoveProduct_${entry.entryNumber}" class="addNewAddLink submitRemoveProduct"><spring:theme code="basket.page.remove"/></a>
									</div>
								</ycommerce:testId>
							</c:if>
						</div>
		...
		<tr class="orderSubTotalRow">
			<c:if test="${not empty freeFrieghtAmt && (logedUserType eq null || logedUserType ne 'ShipTo')}">
				<td class="freeFreightCol" colspan="4">
					<div id="freefrieghtdiv" ${cartData.fmordertype == 'Emergency' ? 'style="display:none"' : 'style="display:block"'}>
						<div class="checkoutSubTitle "><spring:theme code="cart.page.free.frieght.requirement" text="Free Freight Requirement"/></div> 

						<div class="table-responsive userTable">
							<table class="table tablesorter" id="myTable">
								<thead>
									<tr class="text-capitalize" >
										<th><spring:theme code="cart.page.free.frieght.description"/></th>
										<th><spring:theme code="cart.page.free.frieght.AdditionalAmout"/></th>
									</tr>
								</thead>
								...
								<tbody>
									<tr class="freeFrieghtTotal">                				              						    
										...
										<td>
											...
											<c:if test="${freightPrice > freeFrieghtAmt}">
												<div><spring:theme code="cart.page.free.frieght.free"/></div>
											</c:if>
										</td>
									</tr>
								</tbody>
			...
			<c:if test="${not empty freeFrieghtAmt && (logedUserType eq null || logedUserType ne 'ShipTo')}">
				<td colspan="2" class="text-right">
					<p class=""> <span class="shoppingSubTotal"> <spring:theme code="cart.page.subTotal"/></span></p>
				</td>
			</c:if>

			<c:if test="${logedUserType eq 'ShipTo'}">
			...
			<td>
				<p class="shoppingCartTotal fm_fntRed">
...
<div class="modal fade in" id="findPopupLocation" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-Dialog billingShippingPopUpB2B findPopupLocation" style="margin-top: 20.5px; width: 92%;">
		...
		<div class="modal-content col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="modal-header">
				<h2 class="text-uppercase"><spring:theme code="cart.page.pickup.location"/></h2>
			</div>

			<div class="modal-body">
				<div class="form-group  regFormFieldGroup ">
					<label class="" for="billZipPostalCode">
						<spring:theme code="cart.page.pickup.zipcode.city.state"/> <span class="fm_fntRed"><spring:theme code="required" text="*"/></span>
					</label>
				</div>

				<div class="form-group regFormFieldGroup row">
					<div class="col-md-3">
						<input type="text" id="billZipPostalCode" name="billZipPostalCode" placeholder="" class="form-control width270" />
					</div>

					<div class="col-md-2">
						<button class="btn btn-fmDefault shoppingCartCheckOut"><spring:theme code="cart.page.pickup.find.store"/></button>
					</div>
					<!-- <div class="col-md-7">
