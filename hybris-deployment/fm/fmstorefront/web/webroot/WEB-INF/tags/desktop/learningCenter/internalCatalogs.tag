  ...
  <div class="container companyOverview">
    <div class="row"> 
      ...
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12"> 
        ...
        <div class="panel panel-default manageAccountLinks clearfix" >
          <div class="panel-body">
            ...
            <c:forEach var="component" items="${slots.LearningCenterLinksSectionCat.cmsComponents}" varStatus="status">
		
   			<li >
   				<c:url value="/lc/catalogs/${component.uid}?complnkname=${component.name}" var="learningcenterURL" />
   				<c:choose>
					...
					<c:when test="${ (componentUid eq null) and (component.uid eq 'catalogs') }">
   						<a href="${ learningcenterURL}" class="selected"> ${component.name}  <span class="linkarow fa fa-angle-right "></span> </a>
   					</c:when>
  
  		 			<c:when test="${ (componentUid eq null) and (component.uid eq 'training-options') }">
   						<c:set var="component" value="${slots.LearningCenterLinksSectionCat.cmsComponents.get(0)}"></c:set>       	
   					...
   					<c:when test="${ (componentUid eq null) and (component.uid eq 'courses') }">
   						<c:set var="component" value="${slots.LearningCenterLinksSectionCat.cmsComponents.get(1)}"></c:set>       	
   						<a href="${component.content}" target="_blank">${component.name}  <span class="linkarow fa fa-angle-right "></span> </a>
   					</c:when>
   				
   					<c:when test="${ (componentUid eq null) and (component.uid eq 'techtips') }">
   						<c:set var="component" value="${slots.LearningCenterLinksSectionCat.cmsComponents.get(2)}"></c:set>        	
   						<a href="${component.content}" target="_blank">${component.name}  <span class="linkarow fa fa-angle-right "></span> </a>
   					</c:when>
      ...
      <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 internalLanding">
        ...
        <div id=defaultshow >
        <c:forEach begin="0" end="${slots.LearningCenterLinksSectionCat.cmsComponents.size()}" var="component" items="${slots.LearningCenterLinksSectionCat.cmsComponents}">
       ...
       <c:if test="${ componentUid eq null }">
       	<c:set var="component" value="${slots.LearningCenterLinksSectionCat.cmsComponents.get(5)}">
       	</c:set>
