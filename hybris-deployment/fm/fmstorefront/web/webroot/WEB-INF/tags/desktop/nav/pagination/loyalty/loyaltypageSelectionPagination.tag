...
<!-- My Working copy -->
<c:if test="${paginationDisplayDrop eq 'loyaltyDisplay'}">
   ...
   <c:if test="${searchPageData.pagination.totalNumberOfResults > 10 }">
	...
	<c:forEach begin="15" end="45" var="pageNumber" step="15">
...
function goToPageProductCount(url){
	...
	newUrl = newUrl.replace("/c/", "/lc/");
	newUrl = newUrl.replace("/search?", "/lsearch?");
	//alert("URL :: "+newUrl);
...
function goToPageNumber(url){
	...
	//alert("URL :: "+newUrl+"&prodCount="+productsCount);
	document.location.href = newUrl+"&prodCount="+productsCount;
}
