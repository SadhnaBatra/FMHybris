		...
		<div class="media-body">
			...
			<h4 class="text-capitalize">
					...
					<c:when test="${fn:contains(name, '(')}">
							...
							${fn:replace(name, fn:substring(name,fn:indexOf(name,'('),fn:indexOf(name,')')), fn:toUpperCase(fn:trim(name1)))}
					</c:when>
					<c:otherwise>
			...
			<p class="partNoReview">
				<span class="partNo">Part No:</span>
				<c:choose>
				...
				<span class="review">${fn:length(product.reviews)}
					Reviews <span class="linkarow fa fa-angle-right "></span>
				</span>
			</p>
			<c:choose>
			<c:when test="${displayFitmentInfo == 'true'}" >
	...
	<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
		...
		<c:choose>
			<c:when test="${customerType ne 'b2bCustomer' and customerType ne 'b2BCustomer' and customerType ne 'b2cCustomer' and customerType ne 'CSRCustomer' and customerType ne 'FMAdmin'}">
	            <div class=" clearfix">
					<a href="${whereToBuyURI}" class="btn btn-fmDefault pull-left" style="display: block">where to buy</a>
				</div>
			</c:when>
			<c:otherwise>
				<div class=" clearfix">
					<a href="#" onclick="javascript:categoryAddToCart('${product.code}');" class="btn btn-fmDefault pull-left" style="display: block">add to cart</a>
				</div>
				<div id="div_${product.code}" class="col-xs-12">
	...
	<c:if test="${displayFitmentInfo == 'true'}" >
		<div class="">
			<p class="vehiclefitmatch clearfix">
				<span class="fitLabel">Fits:</span><br /> <i
					class="fa fa-check-circle fmFitsChecked"></i> <a href="#">${displayFitmentYear}&nbsp; ${displayFitmentMake }&nbsp;${displayFitmentModel }
					</a>
			</p>
			<c:if test="${displayFitmentInfo == 'true'}" >
