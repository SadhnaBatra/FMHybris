    ...
    <div class="row"> 
      ...
      <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 contentLHS"> 
        ...
        <div class="panel panel-default manageAccountLinks clearfix" >
          ...
          <h3 class="text-uppercase">Suppliers</h3>
              <div class="panel-group sidebar visible-lg visible-md visible-sm productlist-lhs" id="accordion"> 
                ...
                <div class="panel panel-default">
                <c:forEach var="component" items="${slots.CompanySection5.cmsComponents}" varStatus="status">	
                      ...
                      <form class="form-horizontal" role="form">
                        <div class="form-group btmMrgn_0">
                          <ul class="filter-option">
                            ...
                            <c:url value="/about-us/suppliers/${componentList.uid}?complnkname=${componentList.name}&parentCompname=${parentCompname}&parentComplink=${parentComplink}" var="aboutusURLNew" />
                          <c:choose>
				   			<c:when test="${componentList.uid eq componentUid}">				   			
				   				<li><a href="${ aboutusURLNew}" class="selected"><span class="lftMrgn_20">${componentList.name} </span> <span class="linkarow fa fa-angle-right"></span></a> </li>
				   			</c:when>	
				   			
				   			<c:when test="${componentList.name ne component.name}">				   			
				   				<li><a href="${ aboutusURLNew}" ><span class="lftMrgn_20">${componentList.name} </span> <span class="linkarow fa fa-angle-right"></span></a> </li>
				   			</c:when>				   			
...
<div class="panel panel-default manageAccountLinks clearfix">
	<div class="panel-body orgMangPanel">
	     <ul class="">
		<c:forEach var="component" items="${slots.comanysectionsupplier1.cmsComponents}" varStatus="status">
		    <!--<c:url value="/about-us/suppliers/${component.uid}?complnkname=${component.name}" var="aboutusURL" /> 
     ...
     <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 internalLanding">
      	<c:if test="${componentUid eq 'supplieroverview' or componentUid eq null }" >
		<div class="brandingFlyr">
			<!-- <div class="container-fluid"> -->
				<div class="clearfix">
					
						<c:set var="fmComponentName" value="aboutusBrandBanner" scope="session" />
						<cms:pageSlot position="supplierOverviewBrandSection" var="feature">
      ...
      <div class="internalPage rightHandPanel clearfix">
        <div id=defaultshow >
        <c:forEach begin="0" end="${slots.CompanySection5.cmsComponents.size()}" var="component" items="${slots.CompanySection5.cmsComponents}">
       ...
       <c:if test="${ componentUid eq null }">
       	<c:set var="component" value="${slots.CompanySection5.cmsComponents.get(0)}">
       	</c:set>
