...
<%-- searchPageData ::: ${searchPageData.pagination.totalNumberOfResults} --%>
...
<%-- searchResult ::: ${searchResult.getResults().size()} --%>
<c:if test="${searchPageData.pagination.totalNumberOfResults > 0}">
<section class="prodFilter col-lg-12">
		...
		<div class="col-lg-3 col-md-3 col-sm-3 col-xs-2">
			<label class="control-label searchResultText">
			Record Count: <span class="search-result-number">${searchResult.getResults().size()}</span>
				</label>
		</div>
		
	<%-- 	<div class="col-lg-4 col-md-3 col-sm-3 col-xs-4 topMargn_3">
		...
		<%-- <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
			<form class="form-horizontal">
				...
				<c:set var="paginationArrowNav" value="" scope="session" />
					<pagination:wheretobuypageSelectionPagination searchUrl="${searchUrl}" searchPageData="${searchPageData}" numberPagesShown="${numberPagesShown}" themeMsgKey="${themeMsgKey}"/>
				</div>
			</form>
