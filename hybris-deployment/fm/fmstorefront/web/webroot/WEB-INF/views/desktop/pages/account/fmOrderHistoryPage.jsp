...
<section class="breadcrumbPanel visible-lg visible-md visible-sm" itemscope itemtype="http://schema.org/Product">
  <div class="container">
	...
	<div class="container manageAccount" id="orderHistory">
	<div class= "row">
	...
	<c:set var="userGroup" scope="session" value="${userGroup}"/>
	 <c:choose>
		<c:when test="${customerType eq 'b2BCustomer' or customerType eq 'CSRCustomer'}">			
			<federalmogul:orderHistory_B2B />				
		</c:when>
		<c:otherwise>
			<federalmogul:orderHistory />	
		</c:otherwise>
	</c:choose>
	</div>
	</div>
