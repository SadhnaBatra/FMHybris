...
public class CategoryPageController extends AbstractSearchPageController
{
			...
			final HttpServletRequest request, final HttpServletResponse response) throws UnsupportedEncodingException
	{
		...
		{
			appFacetValues.add(facet.getFacetName());
		}
		LOG.info(" Facet SIZE :: " + appFacetValues.size());
		if (appFacetValues.size() > 0)
		{
			if (appFacetValues.contains("year") && appFacetValues.contains("make") && appFacetValues.contains("model")
					&& appFacetValues.contains("vehiclesegment"))
			{
				isYMMSearch = true;
				LOG.info("YMM Applied Flag isYMMApplied :: " + isYMMSearch);
			}
		}


		populateModel(model, searchPageData, showMode);
		model.addAttribute(WebConstants.YMM_SEARCH_FLAG, isYMMSearch);
