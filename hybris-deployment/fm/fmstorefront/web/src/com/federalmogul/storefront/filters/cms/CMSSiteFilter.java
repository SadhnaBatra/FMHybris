 ...
 *  
 */
package com.federalmogul.storefront.filters.cms;
...
{
	/*
	 * @SuppressWarnings("unused") private static final Logger LOG = Logger.getLogger(CMSSiteFilter.class);
	 */
	...
	protected static final String SITE_CMSSITE_PARAM = "site=federalmogul";
	protected static final String LOYALTY_SITE_CMSSITE_PARAM = "site=loyalty";
	protected static final String LOYALTY_SITE_UID = "loyalty";
			...
			throws IOException
	{
		...
		String currentRequestURL = httpRequest.getRequestURL().toString();
		if(currentRequestURL.contains(IFRAME)){
			currentRequestURL=currentRequestURL.replace("/iframe", "/");
			LOG.info("iframe removed from requested url :: " + currentRequestURL);
		}
		...
		{
			LOG.info("CMS SITE ,MODEL" + cmsSiteModel.getUid());
		}
		else
		{
			LOG.info("CMS SITE model is null");
		}
		if (cmsSiteModel != null && cmsSiteModel.getUid() != null && cmsSiteModel.getUid().equals(LOYALTY_SITE_UID)) //StringUtils.contains(currentRequestURL, "SiteRouter"))
		{
			LOG.info("cmsSiteModel.getUid() :: " + cmsSiteModel.getUid());

			if (StringUtils.contains(currentRequestURL, "about-us") || StringUtils.contains(currentRequestURL, "support")
					|| StringUtils.contains(currentRequestURL, "online-tools")
					|| StringUtils.contains(currentRequestURL, "learningCenter") || StringUtils.contains(currentRequestURL, "catalog")	
					|| StringUtils.contains(currentRequestURL, "where-to-buy")
					|| StringUtils.contains(currentRequestURL, "my-fmaccount")
					|| StringUtils.contains(currentRequestURL, "logout")
                                        || StringUtils.contains(currentRequestURL, "brands"))
			{
				final String initialCurReqUrl = currentRequestURL;
				LOG.info("1 ######################################################### ");
				queryString = CLEAR_CMSSITE_PARAM + "&" + SITE_CMSSITE_PARAM;

				currentRequestURL = currentRequestURL.substring(0, currentRequestURL.indexOf("fmstorefront") + 12);

				LOG.info(" currentRequestURL $$$$ " + currentRequestURL);


				final String absoluteURL = StringUtils.removeEnd(currentRequestURL, "/")
						+ (StringUtils.isBlank(queryString) ? "" : "?" + queryString);

				LOG.info("Final absoluteURL :: " + absoluteURL);
				cmsSiteModel = getContextInformationLoader().initializeSiteFromRequest(absoluteURL);
				getCmsSiteService().setCurrentSite(cmsSiteModel);

				cmsSiteModel = getCurrentCmsSite();

				if (StringUtils.contains(initialCurReqUrl , "logout"))
				{
					currentRequestURL = currentRequestURL + "/federalmogul/en/USD/logout";
					LOG.info("Logout URL change " + currentRequestURL );

				}

				LOG.info("site NAme :11: " + cmsSiteModel.getName());
				LOG.info("2 ######################################################### ");
			}
		...
		if (StringUtils.contains(queryString, LOYALTY_SITE_CMSSITE_PARAM))
		{
			queryString = CLEAR_CMSSITE_PARAM + "&" + LOYALTY_SITE_CMSSITE_PARAM;

			final String absoluteURL = StringUtils.removeEnd(currentRequestURL, "/")
					+ (StringUtils.isBlank(queryString) ? "" : "?" + queryString);

			LOG.info("absoluteURL :LL: " + absoluteURL);

			cmsSiteModel = getContextInformationLoader().initializeSiteFromRequest(absoluteURL);
		}
		else if (StringUtils.contains(queryString, SITE_CMSSITE_PARAM))
		{
			queryString = CLEAR_CMSSITE_PARAM + "&" + SITE_CMSSITE_PARAM;
		...
		else if (cmsSiteModel == null || StringUtils.contains(queryString, CLEAR_CMSSITE_PARAM))
		{
			if (StringUtils.isBlank(queryString))
			{
				queryString = SITE_CMSSITE_PARAM;
			}
			else
			{
				queryString += "&" + SITE_CMSSITE_PARAM;
