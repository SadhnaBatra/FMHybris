...
import com.federalmogul.storefront.forms.OrderApprovalDecisionForm;
import com.federalmogul.storefront.forms.QuoteOrderForm;
import com.federalmogul.storefront.forms.ReorderForm;
import com.federalmogul.storefront.forms.UpdateEmailForm;
import com.federalmogul.storefront.forms.UpdatePasswordForm;
import com.federalmogul.storefront.forms.UpdateProfileForm;
...
public class AccountPageController extends AbstractSearchPageController
{
			...
			TokenInvalidatedException
	{
		...
		if (!bindingResult.hasErrors())
		{
			if (updatePasswordForm.getNewPassword().equals(updatePasswordForm.getCheckNewPassword()))
			{
				try
				{
					//customerFacade.updatePassword(token, updatePasswordForm.getNewPassword());
	...
	@RequestMapping(value = "/fmUpdate-password", method = RequestMethod.GET)
	@RequireHardLogIn
	public String fmUpdatePassword(final Model model, final String token) throws CMSItemNotFoundException
	{
		LOG.info("$$$$$$$$$$$$$$$$$111111111---fmupdate=----password----GETTTTTTT---START-");
		final UpdatePwdForm updatePwdForm = new UpdatePwdForm();
		updatePwdForm.setToken(token);

		model.addAttribute("updatePwdForm", updatePwdForm);
		...
		model.addAttribute("metaRobots", "no-index,no-follow");
		//return ControllerConstants.Views.Pages.Account.AccountChangePasswordPage;
		LOG.info("$$$$$$$$$$$$$$22222222222222---fmupdate=----password----GETTTTTTT---END-");
		return ControllerConstants.Views.Pages.Password.FMForgotPasswordResetPage;
	}

	@RequestMapping(value = "/fmUpdate-password", method = RequestMethod.POST)
	@RequireHardLogIn
	public String fmUpdatePassword(@Valid final UpdatePwdForm updatePwdForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException, TokenInvalidatedException
	{
		LOG.info("$$$$$$$$$$$$$$$$$33333333---fmupdate=----password----POST---START-");
		...
		if (!bindingResult.hasErrors())
		{
			...
			LOG.info("$$$$$$$$$$$$$$$$$---fmupdate=----password----post---no binding errors-");
			if (updatePwdForm.getPwd().equals(updatePwdForm.getCheckPwd()))
			{
				try
				{
					customerFacade.updatePassword(updatePwdForm.getToken(), updatePwdForm.getPwd());
					//customerFacade.changePassword(updatePasswordForm.getCurrentPassword(), updatePasswordForm.getNewPassword());
				}
				catch (final PasswordMismatchException localException)
				{
		...
		if (bindingResult.hasErrors())
		{
			LOG.info("$$$$$$$$$$$$$$$$$---fmupdate=----password----post------binding errors----fmupdate=----password----POST---");
			GlobalMessages.addErrorMessage(model, "form.global.error");
			storeCmsPageInModel(model, getContentPageForLabelOrId(FMFORGOT_PASSWORD_RESET_CMS_PAGE));
			setUpMetaDataForContentPage(model, getContentPageForLabelOrId(FMFORGOT_PASSWORD_RESET_CMS_PAGE));
