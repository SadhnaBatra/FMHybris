...
import com.federalmogul.storefront.security.cookie.CartRestoreCookieGenerator;
			...
			final FilterChain filterChain) throws IOException, ServletException
	{
		...
		else
		{
			if ((!getCartFacade().hasSessionCart() && getSessionService().getAttribute(WebConstants.CART_RESTORATION) == null)
					|| (getCartFacade().hasSessionCart() && !getBaseSiteService().getCurrentBaseSite().equals(
							getBaseSiteService().getBaseSiteForUID(cartService.getSessionCart().getSite().getUid()))))
			{
				try
				{

					if (getCartFacade().hasSessionCart() && cartService.getSessionCart().getEntries().isEmpty())
					{
					...
					final CartModel cartForGuidAndSiteAndUser = commerceCartService.getCartForGuidAndSiteAndUser(null,
							getBaseSiteService().getCurrentBaseSite(), getUserService().getCurrentUser());
					parameter.setCart(cartForGuidAndSiteAndUser);

					//getCartRestorationConverter().convert(getCommerceCartService().restoreCart(parameter))
					...
					getSessionService().setAttribute(WebConstants.CART_RESTORATION,
							getCartRestorationConverter().convert(commerceCartService.restoreCart(parameter)));
				}
				catch (final CommerceCartRestorationException e)
				{
					getSessionService().setAttribute(WebConstants.CART_RESTORATION, "basket.restoration.errorMsg");
