...
{
	//private static final Logger LOG = Logger.getLogger(MiniCartController.class);
	/**
			...
			throws CMSItemNotFoundException
	{
		...
		model.addAttribute("logedUserType", sessionService.getAttribute("logedUserType"));
		...
		if (entries != null)
		{
			...
			{
				final FMPartModel part = partService.getPartForCode(entry.getProduct().getCode());
				boolean catalogProduct = true;
				for (final FMCorporateModel corp : part.getCorporate())
				{
				...
				entry.getProduct().setPartNumber(part.getPartNumber());
				entry.getProduct().setRawPartNumber(part.getRawPartNumber());
				if (!catalogProduct)
				{
					entry.getProduct().setCode(part.getRawPartNumber());
				}
				entryies.add(entry);
			}

			model.addAttribute("entries", entryies);
