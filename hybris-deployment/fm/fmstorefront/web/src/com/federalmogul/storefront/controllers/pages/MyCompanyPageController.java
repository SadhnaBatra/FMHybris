...
import com.federalmogul.storefront.forms.B2BBudgetForm;
import com.federalmogul.storefront.forms.B2BCostCenterForm;
import com.federalmogul.storefront.forms.B2BCustomerForm;
import com.federalmogul.storefront.forms.B2BPermissionForm;
import com.federalmogul.storefront.forms.validation.B2BBudgetFormValidator;
import com.federalmogul.storefront.forms.validation.B2BPermissionFormValidator;
...
{
	protected static final String MY_COMPANY_CMS_PAGE = "ManageUserPage";
	protected static final String ORGANIZATION_MANAGEMENT_CMS_PAGE = "CreateUserPage";
	...
	protected static final String REDIRECT_TO_MANAGE_USER = REDIRECT_PREFIX + "/my-company/organization-management/manage-users";
			...
			throws CMSItemNotFoundException
	{
		...
		model.addAttribute("metaRobots", "no-index,no-follow");
		return ControllerConstants.Views.Pages.MyCompany.MyNetworkManageUnitDetailsPage;
	}
	...
	{
		LOG.info("one argument>>>createuser start");
		 ...
		 * Breadcrumb("/my-company/organization-management/manage-users/create", getMessageSource().getMessage(
		 * "text.company.organizationManagement.createuser", null, getI18nService().getCurrentLocale()), null));
		...
		LOG.info("one argument>>>createuser end");
			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		LOG.info("many>>>>>>>>>>>>>>>>>> argument>>>createuser start");
		if (bindingResult.hasErrors())
		{
			GlobalMessages.addErrorMessage(model, "form.global.error");
			model.addAttribute(b2BCustomerForm);
		 ...
		 * Breadcrumb("/my-company/organization-management/manage-user", getMessageSource().getMessage(
	...
	public String editUser(final String user, final Model model) throws CMSItemNotFoundException
	{
		...
		final List<Breadcrumb> breadcrumbs = accountBreadcrumbBuilder.getBreadcrumbs(user);
		 ...
		 * breadcrumbs.add(new Breadcrumb(String.format("/my-company/organization-management/manage-users/edit?user=%s",
		 * urlEncode(user)), getMessageSource().getMessage("text.company.manageusers.edit", new Object[] { user },
		 * "Edit {0} User", getI18nService().getCurrentLocale()), null));
		 */
		...
		LOG.info("in mycompanypagecon>>edituser>>2 args end");
		return ControllerConstants.Views.Pages.MyCompany.MyCompanyManageUserAddEditFormPage;
	...
	protected String manageUserDetail(final String user, final Model model, final HttpServletRequest request)
			throws CMSItemNotFoundException
	{
		LOG.info("in mycompanypagecon>>manageuser>>threee args start>>");
		final CustomerData customerData = companyB2BCommerceFacade.getCustomerDataForUid(user);
	...
	/*
	 * protected List<String> populateRolesCheckBoxes(final List<String> roles) { final List<SelectOption> selectBoxList
	 * = new ArrayList<SelectOption>(); for (final String data : roles) { int i = 1; System.err.println("bfore adding" +
	 * data + i); selectBoxList.add(new SelectOption(data,
	 * getMessageSource().getMessage(String.format("b2busergroup.%s.name", data), null,
	 * getI18nService().getCurrentLocale()))); System.err.println("after adding" + data + i); i++; }
	 * 
	 * final List<String> nameList = new ArrayList<String>(); for (final SelectOption temp : selectBoxList) {
