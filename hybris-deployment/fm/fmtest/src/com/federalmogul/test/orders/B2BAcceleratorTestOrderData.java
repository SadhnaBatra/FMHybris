 ...
 *  
 */
package com.federalmogul.test.orders;
...
import com.federalmogul.test.constants.FmTestConstants;
import com.federalmogul.test.services.AccountManagerApproveScheduleService;
...
public class B2BAcceleratorTestOrderData
{
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(FmTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(FmTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(FmTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(FmTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(FmTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(FmTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(FmTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(FmTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(FmTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(FmTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(FmTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(FmTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(FmTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(FmTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
	...
	protected String placeReplenishmentOrderInCancelledState(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			code = scheduleOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					FmTestConstants.STANDARD_COSTCENTER, null, triggerData).getJobCode();
			if (code != null)
			{
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? FmTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderRequiringApproval(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					FmTestConstants.STANDARD_COSTCENTER, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					FmTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					FmTestConstants.LOWLIMIT_COSTCENTER, null);
	...
	{
		// Lookup the site
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(FmTestConstants.POWERTOOLS_SITE);
		// Lookup the customer
			...
			final String purchaseOrderNumber, final String costCenterCode, final String quotePetition)
	{
		...
		final ImpersonationContext ctx = new ImpersonationContext();
		ctx.setSite(getCmsAdminSiteService().getSiteForId(FmTestConstants.POWERTOOLS_SITE));
		ctx.setUser(customer);
			...
			final B2BCustomerModel customer)
	{
		LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(),
				FmTestConstants.POWERTOOLS_SITE));
		...
		if (containsOrder(orderList, purchaseOrderNumber))
		{
			LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ",
					purchaseOrderNumber, customer.getUid(), FmTestConstants.POWERTOOLS_SITE));
	...
	{
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(FmTestConstants.POWERTOOLS_SITE);
		final B2BCustomerModel customer = getUserService().getUserForUID(FmTestConstants.APPROVER_UID.toLowerCase(),
