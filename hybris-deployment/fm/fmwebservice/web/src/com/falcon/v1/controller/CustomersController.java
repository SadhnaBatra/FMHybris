...
import com.falcon.conv.HttpRequestPaymentInfoPopulator;
import com.falcon.exceptions.RegistrationFailureException;
import com.falcon.populator.HttpRequestAddressDataPopulator;
import com.falcon.populator.HttpRequestCustomerDataPopulator;
import com.falcon.populator.options.PaymentInfoOption;
import com.falcon.user.data.AddressDataList;
import com.falcon.user.data.LoginChangeResponse;
import com.falcon.user.data.PasswordRestoreResponse;
import com.falcon.validation.data.AddressValidationData;
...
public class CustomersController extends BaseController
{
	...
	private static final String REPAIRSHOPOWNER = "REPAIRSHOPOWNER";
	private static final String STUDENT = "STUDENT";
	private static final String TECHNICIAN = "TECHNICIAN";
	private static final String JOBBER = "JOBBER";
	...
	public void registerUser(HttpServletRequest request) throws DuplicateUidException, RegistrationFailureException	{
		LOG.info("inside customersController -> registerUser method");	
		/**
		 * This registerUser rest api will support only REPAIRSHOPOWNER, STUDENT and TECHNICIAN (B2T users) user profiles. It will be enhanced later on by accommodating other users (B2B or B2C)
		 * So when we enhance this api, its better to create a separate populator to populate request parameters.
		 */
		if (request.getParameter("usertype").equalsIgnoreCase((REPAIRSHOPOWNER)) 
				|| request.getParameter("usertype").equalsIgnoreCase((JOBBER)) 
				|| request.getParameter("usertype").equalsIgnoreCase((STUDENT)) 
				|| request.getParameter("usertype").equalsIgnoreCase((TECHNICIAN))) {
			
			LOG.info("registerUser: login=" + request.getParameter("email"));
			if (LOG.isDebugEnabled()) {
				LOG.debug("registerUser: login=" + request.getParameter("email"));
			}
	
			if (!EmailValidator.getInstance().isValid(request.getParameter("email"))) {
				throw new RequestParameterException("Login [" + request.getParameter("email") + "] is not a valid e-mail address!",
						RequestParameterException.INVALID, "login");
			}
			if (fmCustomerFacade.checkUidExists(request.getParameter("email"))) {
				LOG.info("An account with this email already exists, please register with a different email address.");
				throw new DuplicateUidException("An account with this email already exists, please register with a different email address.");
			}
			
				final FMCustomerData fmCustomerData= new FMCustomerData();
				fmCustomerData.setFirstName(request.getParameter("firstname"));
				fmCustomerData.setLastName(request.getParameter("lastname"));
				fmCustomerData.setLogin(request.getParameter("email"));
				fmCustomerData.setEmail(request.getParameter("email"));
				fmCustomerData.setPassword(request.getParameter("password"));
				fmCustomerData.setTitleCode(request.getParameter("titlecode"));
				
				if (request.getParameter("usertype").equalsIgnoreCase(("RepairShopOwner"))) {
					fmCustomerData.setUserTypeDescription(Fmusertype.REPAIRSHOPOWNER);
				}
				else if (request.getParameter("usertype").equalsIgnoreCase(("Student"))) {
					fmCustomerData.setUserTypeDescription(Fmusertype.STUDENT);
				}
				else {		
					fmCustomerData.setUserTypeDescription(Fmusertype.TECHNICIAN);
				}
				AddressData customerAddressData = new AddressData();
				FMB2bAddressData fmB2bAddressData = new FMB2bAddressData();
				//fmB2bAddressData.setLine1(request.getParameter("addressLine1"));
				//fmB2bAddressData.setLine2(request.getParameter("addressLine2"));
				//fmB2bAddressData.setTown(request.getParameter("city"));
				
				//customerAddressData.setLine1(request.getParameter("addressLine1"));
				//customerAddressData.setLine2(request.getParameter("addressLine2"));
				//customerAddressData.setTown(request.getParameter("city"));
				
				//RegionData regionData = new RegionData();
				//regionData.setIsocodeShort(request.getParameter("state"));
				//regionData.setIsocode(request.getParameter("state"));
				//customerAddressData.setRegion(regionData);
				
				CountryData countryData = new CountryData();
				countryData.setIsocode(request.getParameter("country"));
				
				customerAddressData.setCountry(countryData);
				customerAddressData.setPostalCode(request.getParameter("zipcode"));
				fmCustomerData.setDefaultShippingAddress(customerAddressData);
				
				//fmB2bAddressData.setRegion(regionData);
				
				fmB2bAddressData.setCountry(countryData);
				fmB2bAddressData.setPostalCode(request.getParameter("zipcode"));
				
				fmCustomerData.setB2baddress(fmB2bAddressData);
				//fmCustomerData.setDefaultBillingAddress(customerAddressData);
				
				fmCustomerData.setNewProductAlerts(new Boolean(request.getParameter("new_product_alert")));
				fmCustomerData.setIsGarageRewardMember(new Boolean(request.getParameter("garage_reward_member")));
				
				if (fmCustomerData.getIsGarageRewardMember()) {
					fmCustomerData.setPromoCode(request.getParameter("promocode"));	
				}
				else {
					fmCustomerData.setPromoCode(null);
				}
				LOG.info(fmCustomerData.getEmail()+":"+fmCustomerData.getFirstName()+":"+fmCustomerData.getLastName()+":"+fmCustomerData.getPromoCode()+":"+fmCustomerData.getPassword()+":"+fmCustomerData.getUserTypeDescription()+":"+fmCustomerData.getIsGarageRewardMember());
					
				try {
					fmCustomerFacade.createCustomerAccount(fmCustomerData);
				}
				catch(Exception e) {
					LOG.debug("Error occurred while registration of new user:" + e.getMessage());
					e.printStackTrace();
					throw new RegistrationFailureException("New user registration failed. "+e.getMessage());
				}	

				LOG.info("New user registration completed successfully!.");
				}
		}
	...
	@ResponseBody
	public FMCustomerData updateProfile(final HttpServletRequest request) throws DuplicateUidException
	{
		try{
			LOG.info("inside : CustomerController->updateProfile");
			final FMCustomerData customer = getCurrentCustomer();
			if (LOG.isDebugEnabled()) {
				LOG.debug("updateCustomer: userId=" + customer.getUid());
			}
			httpRequestCustomerDataPopulator.populate(request, customer);
			fmCustomerFacade.updateFMProfile(customer);
		}
		catch (ClassNotFoundException cnfe) {
			LOG.debug("ClassNotFoundException:" + cnfe.getMessage());
		...
		return fmCustomerFacade.getCurrentFMCustomer();
	}
	...
	{
		LOG.info("inside : CustomersController->getAddresses()");
	...
	@ResponseBody
	public FMCustomerData getCurrentCustomer()
	{
		FMCustomerData fmCustomer = fmCustomerFacade.getCurrentFMCustomer();
		return fmCustomer;
	}
