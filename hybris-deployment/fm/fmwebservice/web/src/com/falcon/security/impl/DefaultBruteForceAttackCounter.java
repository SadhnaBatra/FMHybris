 ...
 *  
 */
package com.falcon.security.impl;


import com.falcon.security.BruteForceAttackCounter;
...
public class DefaultBruteForceAttackCounter implements BruteForceAttackCounter
{
	...
	public void registerLoginFailure(final String userUid)
	{
		...
		{
			LOG.info("Registering a login failure for " + userUid);
			...
			LOG.info("Login failure count set to " + count.getCounter() + " for user " + userUid);
	...
	public boolean isAttack(final String userUid)
	{
		if (StringUtils.isNotEmpty(userUid))
		{
			boolean isAttack = maxFailedLogins.compareTo(get(prepareUserUid(userUid), Integer.valueOf(0)).getCounter()) <= 0;
			LOG.info("Is this login request for user " + userUid  + " an attack?: " + isAttack);
			return isAttack;
		}
		else
		{
