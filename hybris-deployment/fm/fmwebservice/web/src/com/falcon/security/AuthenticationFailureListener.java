...
public class AuthenticationFailureListener implements ApplicationListener<AuthenticationFailureBadCredentialsEvent>
{
	...
	{
		final Authentication auth = ev.getAuthentication();
		...
		{
			LOG.info("bruteForceAttackCounter determined an attack for user " + userName);
	...
	protected void disableUser(final String user)
	{
		try
		{
			...
			if (!userModel.isLoginDisabled())
			{
				...
				LOG.info("Account for user '" + user + "' was disabled because of too many authentication failures");
			}
			bruteForceAttackCounter.resetUserCounter(user);
			LOG.info("bruteForceAttackCounter was reset");
		}
		catch (final UnknownIdentifierException e)
		{
			LOG.warn("Brute force attack attempt for non existing user name " + user);
		}
	}
